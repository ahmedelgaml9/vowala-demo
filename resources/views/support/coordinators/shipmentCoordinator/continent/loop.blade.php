...
<tr id="trow_{{ $row->id }}">
 ...
 <!--        <button class="btn btn-danger btn-rounded btn-sm" onClick="delete_row('trow_{{ $row->id }}');"><span class="fa fa-times"></span></button>-->
        {!! Form::open(['action'=>['Coordinators\shipmentCoordinator\ContinentController@destroy',$row->id],'method'=>'delete' ,'style'=>'display: inline']) !!}
...
<div id="modal{{ $row->id }}" class="modal">
    <div class="modal-content">
        {!! Form:: model($row,array('method' => 'PATCH','action' => ['Coordinators\shipmentCoordinator\ContinentController@update',$row->id], 'files'=>true,'class' => 'ajax-form-request')) !!}
