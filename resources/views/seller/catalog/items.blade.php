...
<tr id="trow_{{ $row->id }}">
  ...
  <!--        <button class="btn btn-danger btn-rounded btn-sm" onClick="delete_row('trow_{{ $row->id }}');"><span class="fa fa-times"></span></button>-->
        {!! Form::open(['action'=>['ProductController@delspec',$row->id],'method'=>'delete' ,'style'=>'display: inline']) !!}
...
<div id="modal{{ $row->id }}" class="modal">
    <div class="modal-content">
        ...
        {!! Form:: model($row,array('method' => 'PUT','action' => ['DetailsController@update',$row->id], 'files'=>true,'class' => 'ajax-form-request')) !!}
        <div class="card">
